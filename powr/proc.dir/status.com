...
then
 ...
 then
    ...
    echo 'stop' > $POWR_WORKu/scratch/wruniq${jobnumber}/fbreak
 fi
 scratchwhere=${scratchwhere}${jobnumber}
 killhost=`cat $POWR_WORKu/scratch/${scratchwhere}/fwhere`
 if [[ "_$killhost" == "_$HOSTNAME" ]]
 then
   echo "Sending local KILL"
   kill -KILL `ps axu | grep $USER | grep ${jobnumber}_${killprog}  | grep -v 'grep' | head -1 | awk '{print \$2}'`
 else
...
then
  ...
  st=`cat $POWR_WORKu/scratch/$k/status`
  ...
  cp $POWR_WORKu/wrdata$kn/CARDS /home/$fw/tmp_data/$us/$k/CARDS
  exit
fi
