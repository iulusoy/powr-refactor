#!/bin/bash
#fully automatic update of levelcards after a new DATOM has been generated (meta-script)
...
if [ -e CARDS ]; then
  ...
  if [ -z "$POWRVHDPATH" ]; then
    levelcardsexe=$POWREXEPATH"/levelcards.exe.opt"
  else
    levelcardsexe=$POWRVHDPATH"/levelcards.exe.opt"
  fi
