...
unset usera
set usemodel=no
set postscript=false
set currdate=`date +"%y%m%d%H%M%S"`
if ($#argv == 1) then
 if ($1 == +) then
  set postscript=true
...
if ( $usemodel == no ) then
 if ( -d /home/`cat $POWR_WORK/scratch/wruniq${kn}/fwhere`/tmp_data/${usera}/wruniq${kn} ) then
  cd /home/`cat $POWR_WORK/scratch/wruniq${kn}/fwhere`/tmp_data/${usera}/wruniq${kn} 
 else if ( $POWR_INSTTYPE == local) then
  cd $POWR_TMP_DATA/wruniq$1
 else
  cd $POWR_WORK/scratch/wrstart${kn}
  set rtmphost=`cat $POWR_WORK/scratch/wruniq$1/fwhere`
  scp ${rtmphost}:/home/${rtmphost}/tmp_data/${usera}/wruniq$1/out .
  scp ${rtmphost}:/home/${rtmphost}/tmp_data/${usera}/wruniq$1/steal.plot .
  scp ${rtmphost}:/home/${rtmphost}/tmp_data/${usera}/wruniq$1/como.plot .
  /bin/cp -f $POWR_WORK/wrdata$1/CARDS .  
 endif
 set wruniqpath=$PWD
 set wruniqfile=/steal.plot
else # usemodel = yes 
 set wruniqpath=$PWD
 set wruniqfile=/wruniq.plot
endif
## {unset usera && exit}
...
set plotname=emergentflux$USER.plot
set plotfile=$tmppath/$plotname
/bin/rm -f $plotfile
# -- CREATe PLOT FILE --
