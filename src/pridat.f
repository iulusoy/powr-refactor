...
C***  Define and append the autoionization levels
ccc   Now already in DATOM!
ccc      IF (NAUTO .GT. 0) 
ccc     >    CALL APPEND_AUTOLEVELS (N, N_WITH_DRLEVELS, NDIM, MAXIND, 
ccc     >            MAXAUTO, NAUTO, LOWAUTO, IONAUTO, EAUTO, ELEVEL,
ccc     $            LEVEL, EION, WEIGHT, INDLOW, INDNUP, LASTIND,
ccc     >            LEVUPAUTO, LEVAUTO, WAUTO, NCHARG, IONGRND, NOM)
...
C***  LINE TRANSITIONS  ------------------------------------------------
    ...
    4 FORMAT (//,10X,'2. LINE TRANSITIONS :',/,10X,20('-'),//,
     $ '   IND   UP  LOW   UPPER LEV    LOWER LEV WAVELENGTH   A(UP-LOW)',
     $ 2X,'F(LOW-UP)',12X,'COLLISIONAL  TRANSITIONS',/,
     $ '   (INDICES)      (CONFIGURATIONS)       (ANGSTROEM) (PER SECOND)'
     $ ,18X,'(KEYWORD)',10X,'(COEFFICIENTS)',/)
      DO 1 IND=1,LASTINDALL
C***    Skip stabilizing lines here
        IF (IND .GT. LASTIND .AND. IND .LE. LASTIND+NAUTO) CYCLE
      I=INDNUP(IND)
...
C***  SHORTENING OF OUTPUT:  VALUES==0.0 ARE OMITTED
      ...
      ENDIF
   17 CONTINUE
      PRINT 6, IND,I,J,LEVEL(I),LEVEL(J),WLENG,EINST(I,J),CHF,KRUDI,
     $         KEYCBB(IND),(CHCOCO(M),M=1,4)
    6 FORMAT(1X,3I5,3X,A10,3X,A10,F10.2,1PE12.2,2X,A9,3X,A3,5X,A4,
     $       4(2X,A8))
    1 CONTINUE
...
C***  Skip if there is not any DRTRANSIT with current element index NA
      DO 31 I=1,NAUTO
   ...
   90 FORMAT (//,10X,'4. DR-LEVELS :',/,10X,14('-'),/,
     $ '  NR      NAME       WEIGHT  ENERGY(REL.)  ENERGY    ', 
     $ 'CHARGE   PARENT ION', /,30X,'(KAYSER)',/)

      DO J=N+1, N_WITH_DRLEVELS
         IF (NOM(J) .NE. NA) CYCLE
         NW=IFIX(WEIGHT(J))
         PRINT 92, J, LEVEL(J), NW, ELEVEL(J)-EION(J), 
     >       ELEVEL(J), NCHARG(J), LEVEL(IONGRND(J))
   92    FORMAT (I4,3X,A10,I10,2F11.2,I10,3X,A10)
      ENDDO
   ...
   33 FORMAT (//,10X,'5. STABILIZING LINE TRANSITIONS',/,
     ...
     $ ' (PER SECOND)',/)
      ...
      DO 35 INDDR=1,NAUTO
         ...
         LOW = INDLOW(IND)
         XLAM0 = 1.E8 /
     >              (ELEVEL(NUP)-ELEVEL(LOW))
cc     >              (EION(LOW) - ELEVEL(LOW) + EAUTO(INDDR))
      FVALUE = C4 * XLAM0 * XLAM0 * AAUTO(INDDR) * 
     *         WEIGHT(NUP) / WEIGHT(LOW)
      WRITE (CHF,'(1PG9.3)') FVALUE
      DRRUDI='   '
      IF (KRUDAUT(INDDR) .EQ. 1) DRRUDI=' X '
      IF (NOM(LOW) .EQ. NA) PRINT 34, 
     >        IND, NUP, LOW, INDDR, LEVUPAUTO(INDDR), 
     >        LEVEL(LOW), XLAM0, AAUTO(INDDR), CHF, DRRUDI
   34 FORMAT(1X,3I5,1X,I4,3X,A10,3X,A10,F10.2,1PE12.2,2X,A9,2X,A3)
   35 CONTINUE
