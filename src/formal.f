...
C***  NDIM = Number-of-original-levels + MAXSUBLevels (without 2*)
      ...
      INTEGER, PARAMETER :: NFLDIM  =          400000   
      INTEGER, PARAMETER :: NFODIM  =          500000   
...
C***  VECTORS FOR USE IN SUBR. ZONEINT
     ...
     >                          SFINE, CSFINE, ZFINE
      ...
      LOGICAL, DIMENSION(NDDIM, MAXMOD) :: bFELASER
...
ccc      REAL, DIMENSION(INDEXMAX) :: SIGMAFE, SIGMAFEUL, SIGNU3FE
     ...
     >                            INDFESACT, IFES
      ...
      REAL, DIMENSION(NDDIM,MAXION,MAXMOD) :: OPAFEION, ETAFEION
...
C***  Initialize bFELASER (updated by CMFFEOP)
      bFELASER = .FALSE.
...
C *** IRON: Additional Parameter used in SUBROUTINE CMFFEOP called from FORMCMF
     ...
     >        POPMIN(IMOD), INDEXMAX, MAXION, OPAFEION(1,1,IMOD), 
     >        ETAFEION(1,1,IMOD), bFELASER(1,IMOD), bNoIronLaser)   
