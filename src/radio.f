...
C***  GETTING LINE RADIATION FIELD FOR STABILIZING TRANSITION
      IF (KRUDAUT(J) .EQ. 1) THEN
C***     DR-LINE IS RUDIMENTAL: INTERPOLAION ON BACKGROUND
C***     Note: ND and L = 1 because XJC is one-dimensional here
         CALL XRUDI (XJLSTAB,WSTABIL,XJC,XLAMBDA,ND,NF,L)
      ELSE
C***     DRTRANSIT was calculated in COLI and now read from MODEL
