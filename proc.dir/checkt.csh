...
if ( $usemodel == no ) then
 if ( -d /home/`cat ${POWR_WORKu}/scratch/wruniq${kn}/fwhere`/tmp_data/${usera}/wruniq${kn} ) then
  ...
  set stealpath=/home/`cat ${POWR_WORKu}/scratch/wruniq${kn}/fwhere`/tmp_data/${usera}/wruniq${kn}
 else
...
set rconline1=`egrep -m 1 -B1 "?LUNA [1-99].* R&Tcon" $stealfile | head -1`
set rconline2=`egrep -m 1 -B1 "?LUNA [1-99].* R&Tcon" $stealfile | tail -1`
echo $rconline1 >> $plotfile
echo $rconline2 >> $plotfile
if ( `grep UNLU $stealfile` == '') then
